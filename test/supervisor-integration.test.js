...
describe('Supervisor Integration', function () {
  ...
  afterEach(async function () {
    if (supervisorProcess && !supervisorProcess.killed) {
      // Try graceful IPC shutdown. Use callback form so Windows async ERR_IPC_CHANNEL_CLOSED
      // is routed to the callback rather than thrown from a setImmediate callback (uncatchable).
      try {
        if (supervisorProcess.connected) {
          supervisorProcess.send({ type: 'shutdown' }, () => { /* swallow IPC errors */ });
        }
      } catch (_) { /* ignore synchronous errors (e.g. channel already closed) */ }
