...
- **Session-scoped shell integration** (per [ADR-0050](../adrs/0050-session-scoped-shell-integration.md) and [ADR-0054](../adrs/0054-platform-specific-shell-integration-readiness.md)). Before spawning PowerShell, bash, or zsh, `TerminalBridge` creates a private transient shim below `os.tmpdir()/.ai-or-die-shell/` and supplies it through `-NoExit -File`, `--rcfile`, or `ZDOTDIR`. Each shim sources the user's normal profile/rc first and appends or wraps the existing prompt; it never writes the user's files. The PowerShell startup script preserves profile functions, aliases, variables, and modules in the interactive scope. PowerShell emits empty-host drive URIs and host-preserving UNC URIs; all shims percent-encode path bytes. fish remains native and `cmd.exe` remains unsupported. A ready sentinel detects an early wrapper exit or a startup that does not become ready within two seconds for POSIX/non-PowerShell integrations or within ten seconds for Windows PowerShell, then causes an automatic vanilla-shell respawn. The readiness loop polls every 25 ms and accepts an optional `{ now, sleep }` clock object for focused deterministic tests. Output retained during that readiness window is capped at 256 KiB. Explicit stop/cleanup cancels the pending start instead of respawning. Temp artifacts and OSC state are removed on explicit stop, natural exit, error, and full cleanup; `AIORDIE_DISABLE_SHELL_INTEGRATION=1` disables integration.
