...
- **Session-scoped shell integration** (per [ADR-0050](../adrs/0050-session-scoped-shell-integration.md)). Before spawning PowerShell, bash, or zsh, `TerminalBridge` creates a private transient shim below `os.tmpdir()/.ai-or-die-shell/` and supplies it through a dot-sourced `-Command`, `--rcfile`, or `ZDOTDIR`. Each shim sources the user's normal profile/rc first and appends or wraps the existing prompt; it never writes the user's files. Dot-sourcing the PowerShell shim preserves profile functions, aliases, variables, and modules in the interactive scope. PowerShell emits empty-host drive URIs and host-preserving UNC URIs; all shims percent-encode path bytes. fish remains native and `cmd.exe` remains unsupported. A ready sentinel detects an early wrapper exit and causes an automatic vanilla-shell respawn. Temp artifacts are removed by `stopSession`/`cleanup`; `AIORDIE_DISABLE_SHELL_INTEGRATION=1` disables integration.
