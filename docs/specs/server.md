...
| `join_session` | Join an existing session. Fields: `sessionId`. Replays the newest stored output up to the full live-buffer cap (512 KiB / 1000 lines, `CircularBuffer.LIVE_OUTPUT_MAX_BYTES`). When the headless transcript reports the session is in the alternate screen but the tail no longer contains the alt-enter (evicted from the ring), the replay prepends `ESC[?1049h` so fullscreen TUIs reconverge instead of drawing into the normal buffer. When the transcript reports live mouse tracking but the tail lost the enable, the replay re-asserts the class-minimal set (`vt200 → 1000h+1006h`, `drag → +1002h`, `any → +1003h`) after the alt-enter, so wheel/click reporting survives the client's RIS-clear. |
| `request_repaint` | Ask a live fullscreen TUI to repaint from its own model. Fields: `sessionId`, `reason`. The server runs a +1 bump round-trip through the PTY (transient cols+1, then back to the committed grid — a same-size resize delivers no signal) inside the geometry output hold, gated on live + alt-screen + known grid + a 2s per-session rate limit, and acks `repaint_assisted{ok, reason, roundTrip}`. |
...
`broadcastToSession(sessionId, data)` iterates the session's `connections` Set, verifies each WebSocket is open and belongs to the correct session, then sends the message. The payload is JSON-stringified once per broadcast and shared across viewers; `output`-type deltas are shed for sockets with more than 512 KiB buffered. Coalesced output for sessions with zero live viewers is discarded before join/encode (the `outputBuffer` retains the bytes for a later full replay).
