...
| `request_repaint` | Ask a live fullscreen TUI to repaint from its own model. Fields: `sessionId`, `reason`. The server re-applies the session's current geometry through the PTY (same-size resize = real SIGWINCH) inside the geometry output hold, gated on live + alt-screen + known grid + a 2s per-session rate limit, and acks `repaint_assisted{ok, reason}`. |
