...
- **File-browser contract gate** -- the mobile-journey job also runs the complete `file-browser-v2` project, including artifact review and terminal click-to-open behavior. Spec 69 drives a real bash Terminal session, changes into a directory containing a space, waits for the session-scoped shell integration to update `liveCwd`, and activates the registered xterm link provider without manually emitting OSC 7. PowerShell and zsh session wrappers are covered by the real-shell integration suite on their available platforms.
