...
Cortex provides optional token-based authentication for both HTTP and WebSocket connections. Authentication is managed by two independent classes: a server-side `AuthManager` utility and a client-side `AuthManager` in the browser.
