...
- **Out of scope (deferred)**:
  ...
  - **UI feedback on `validatePath()` rejection.** When the user `cd`s outside the `--folder` sandbox, the OSC 7 emit is silently dropped (per the security rule in Parser semantics above) and the file-browser panel freezes at the last in-sandbox CWD. v1 ships the silent-reject behaviour as-documented; users will see the panel "freeze" and may think OSC 7 is broken. Workaround documented in `file-browser.md` Limitations: `cd` back inside the sandbox, OR restart with a wider `--folder` scope. A Phase 2 follow-up could surface a small toast or a hint near the "📍 follow terminal" toggle (e.g. `"OSC 7 emit for /etc rejected — outside sandbox"`); flagged as a finding from QA's user-journey review (task #15).
