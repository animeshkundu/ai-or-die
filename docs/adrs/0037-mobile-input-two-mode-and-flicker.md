...
- CI must `npx playwright install webkit` on ubuntu (`test-browser-ios-webkit` job); e2e specs 77-79 run under the WebKit engine on iPhone 16 + iPad (gen 11) projects. **Amended by ADR-0049:** this originally read "ubuntu + windows". Playwright-WebKit on the Windows runner wedges inbound WebSocket frame delivery and its workers must be force-killed at teardown, so the job reports failure even when every test in it passes. Windows users run Edge (Chromium), covered by the `test-browser-*` matrix on `windows-latest`.
