...
const VIEWPORTS = {
  ...
  phone: { width: 393, height: 852, isMobile: true, hasTouch: true, deviceScaleFactor: 3 },
};
  ...
  try {
    ...
    for (const theme of [null, 'light']) {
      await withPage(VIEWPORTS.desktop, async (page) => {
        ...
        await settle(page);
        await shot(page, `01-welcome-desktop-${theme || 'dark'}`);
      });
    }
    ...
    await withPage(VIEWPORTS.desktop, async (page) => {
      ...
      await settle(page);
      await shot(page, '02-terminal-active-desktop');

      // 4. Settings modal.
      await page.click('#settingsBtn');
      await page.waitForSelector('.settings-modal.active', { timeout: 8000 });
      await settle(page);
      await shot(page, '03-settings-modal-desktop');
      await page.keyboard.press('Escape').catch(() => {});
      await page.waitForTimeout(400);

      // 5. File browser panel.
      const fbBtn = await page.$('#fileBrowserBtn, #filesBtn, [data-action="file-browser"]');
      if (fbBtn) {
        await fbBtn.click().catch(() => {});
        await page.waitForTimeout(1600);
        await settle(page);
        await shot(page, '04-file-browser-desktop');
      }
    });
    ...
    await withPage(VIEWPORTS.phone, async (page) => {
      ...
      await page.waitForTimeout(2500);
      await settle(page);
      await shot(page, '06-terminal-active-phone');
    });
  } finally {
