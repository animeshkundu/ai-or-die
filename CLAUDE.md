...
- `src/keepalive-manager.js`. While the server runs it holds two redundant Windows power assertions via one persistent in-box `powershell.exe` helper that blocks on stdin and releases on EOF (graceful or parent death): (1) `SetThreadExecutionState` and (2) a named `PowerCreateRequest`/`PowerSetRequest` whose `REASON_CONTEXT` reason string is `GitHub Copilot CLI session active` — the exact API + string the GitHub Copilot CLI `/keep-alive` uses, so `powercfg /requests` reads as Copilot (the holding process stays `powershell.exe`; the label is cosmetic). No deps. ON by default on Windows (`--no-keepalive` / `AIORDIE_DISABLE_KEEPALIVE=1` disables); instant no-op on macOS/Linux. System-awake only; `--keepalive-display` also holds the display. Degrades to a visible warning under WDAC/CLM/EDR. See ADR-0029 (supersedes ADR-0028), `docs/specs/keepalive.md`.
- `src/hibernation-guard.js`. The wake assertion only blocks the *idle timer* — it cannot stop a host-initiated hibernate (on a Hyper-V guest the host's `vmicshutdown` requests hibernation via the Application API). Opt-in `--disable-hibernation` / `AIORDIE_DISABLE_HIBERNATION=1` (OFF by default) runs a one-shot at startup: detect `HibernateEnabled` (non-privileged; SKIP + no UAC if already off), else one elevated `cmd.exe` via `Start-Process -Verb RunAs` runs `powercfg /hibernate off` + sleep/hibernate timeouts→Never under a single UAC prompt. Best-effort + non-fatal (declined UAC / headless / spawn failure → warn, startup continues); persistent (reverse with `powercfg /hibernate on`). Windows only; skipped under test/CI. See ADR-0030, `docs/specs/keepalive.md`.
